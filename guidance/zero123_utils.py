...
class Zero123(nn.Module):
    def __init__(self, device, fp16=True, t_range=[0.02, 0.98], model_key="", img_size=256):
        super().__init__()
        ...
        self.num_train_timesteps = self.scheduler.config.num_train_timesteps #1000
    ...
    def refine(self, pred_rgb, elevation, azimuth, radius, 
        ...
        if strength == 0:
            ...
            pred_rgb_256 = F.interpolate(pred_rgb, (self.img_size, self.img_size), mode='bilinear', align_corners=False)
        ...
        # img = imgs[0].permute(1,2,0).detach().cpu().numpy()*255
        # # print(elevation, azimuth)
        # os.makedirs('./logs/img_priors2/',exist_ok=True)
        # imageio.imwrite('./logs/img_priors2/%d_%d.jpg'%(elevation[0],azimuth[0]), img.astype(np.uint8))
    ...
    def train_step(self, pred_rgb, elevation, azimuth, radius, step_ratio=None, guidance_scale=5, as_latent=False, default_elevation=0):
        ...
        # latents = self.scheduler.step(noise_pred, t, latents).prev_sample
        # imgs = self.decode_latents(noise_pred) # [1, 3, 256, 256]
        # imgs = self.decode_latents(noise_pred_cond) # [1, 3, 256, 256]
        # img = imgs[0].permute(1,2,0).detach().cpu().numpy()*255
        # # print(elevation, azimuth)
        # os.makedirs('./logs/hair2/img_priors/',exist_ok=True)
        # imageio.imwrite('./logs/hair2/img_priors/%d_%d.jpg'%(elevation[0],azimuth[0]), img.astype(np.uint8))
        # loss=0.0
