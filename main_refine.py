...
class Main:
    ...
    def update_targets(self, stage_id, strength=None, need_diffusion=True, save=False):
        print('stage %d'%stage_id)
        if strength is None and need_diffusion:
            strength = 0.5 + (0.8-0.5)/(self.stage-1)*stage_id
        ...
        for orbit_id in range(int(180/counter_num)):
            ...
            if need_diffusion:
                ...
                self.refined_images.append(refined_images.detach().cpu().numpy())
            ...
            if save:
                ...
                for counter_id in range(counter_num):
                    ...
                    os.makedirs(self.opt.outdir + '/%sgs_refine_render/'%self.save_prefix, exist_ok=True)
                    imageio.imwrite(self.opt.outdir + '/%sgs_refine_render/%s.png'%(self.save_prefix, str(save_id).zfill(4)), out_img.astype(np.uint8))
    ...
    def save_model(self):
        ...
        path = os.path.join(self.opt.outdir, self.opt.save_path + '_refine.ply')
