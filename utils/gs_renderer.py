...
class GaussianModel:
    ...
    def update_learning_rate_extra(self):
        ''' Learning rate scheduling per step '''
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "f_dc":
                param_group['lr'] = param_group['lr']*10
                print('f_dc lr:', param_group['lr'])

    def set_position_learning_rate(self, lr=0.0):
        ''' Learning rate scheduling per step '''
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                param_group['lr'] = lr
    ...
    def reset_scale(self):
        dist2 = torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(self.load_xyz)).float().cuda()), 0.0000001)
        scales = torch.log(torch.sqrt(dist2))[...,None].repeat(1, 3)
        self._scaling = nn.Parameter(scales.requires_grad_(True))
...
class Renderer:
    ...
    def initialize(self, input=None, num_pts=5000, radius=0.5):
        ...
        if input is None:
            ...
            xyz[:,0] = xyz[:, 0]* 0.4 - 0.2
            xyz[:,1] = xyz[:, 1]* 0.6 - 0.3
            xyz[:,2] = xyz[:, 2]* 0.3 - 0.15

            # save_obj('./ga_init_pts.obj', xyz)
